...
play_state ShopRoom::play(int choice)
{
	...
	if(!enoughGold){
		statusString->at(1) = "Not enough gold";
	...
	if(choice == -1)
	{
		...
		statusString->at(0) = to_string(heal.first) + " life " + to_string(heal.second) + "g";
		statusString->at(2) =to_string(boost.first) + " zooms " + to_string(boost.second) + "g";
		statusString->at(1) =(ShopEquip->description() + ShopEquip->get_name())
				+ to_string(ShopEquip->get_value()) + "g";
		return play_state::OFFER;
	}
	else if(choice == 1 && enoughGold){
		main->setCurrentHP(main->getMaxHP() + heal.first);
		statusString->at(0) = "Healed " + to_string( heal.second);
		main->setGold(-heal.second);
		return play_state::DONE;

	}
	else if(choice == 2 && enoughGold){
		main->boostStats(boost.first);
		statusString->at(0) = "All stats up !" + to_string(boost.first);
		main->setGold(-boost.second);
		return play_state::DONE;
	}
	else if(choice == 3 && enoughGold){
		statusString->at(0) = "Bought a " + ShopEquip->description() + ShopEquip->get_name();
...
{
	int amount = main->getMaxHP() * 0.25 + 1;
...
void ShopRoom::generateBoost()
{
	...
	int amount = RANDOM_INT(1, level/10 +2);
	int gold = pow(2,level/3) + RANDOM_INT(level * 50, level*100);
